...
#   2. SecretRef — pre-existing secrets for token + registry
#   3. SecretRef + own registry — same as 2, different values file
#
...
create_pre_existing_secrets() {
  # Creates the token and registry secrets that the secretRef path requires.
  # In production, these would be created by ExternalSecrets or manually.
...
# ==========================================================================
# TEST 2: SecretRef (pre-existing secrets)
# ==========================================================================
info "TEST 2: SecretRef path (pre-existing token + registry secrets)"
cleanup
kubectl create namespace "$NAMESPACE"

# Create pre-existing secrets (simulates ExternalSecrets or manual creation)
create_pre_existing_secrets
...
# Verify chart did NOT create its own secrets
if ! check_secret_exists "entitle-agent-secret"; then
...
if ! check_secret_exists "entitle-agent-docker-login"; then
  pass "Test 2: no chart-managed docker-login (expected with imagePullSecret.name)"
else
  fail "Test 2: chart-managed docker-login exists unexpectedly"
fi
...
if check_deployment_image_pull_secret "entitle-agent-ci-registry"; then
  pass "Test 2: imagePullSecrets correct (pre-existing)"
else
...
# ==========================================================================
# TEST 3: SecretRef + own registry (same secrets, different values file)
# ==========================================================================
